...
import { HeartFilled, HeartOutline } from "@/components/passport-heart-icons";
...
export function BrandPreferencesStep({
  initialBrands,
  onStepCompleteChange,
  /** When true, the page heading + lead are omitted (e.g. profile supplies its own) */
  ...
  const [portfolio, setPortfolio] = useState<string[]>(initialBrands);
  ...
  useEffect(() => {
    onStepCompleteChange?.(portfolio.length > 0);
  }, [portfolio, onStepCompleteChange]);
  ...
  function scrollToSection(letter: string) {
    const id = letter === "#" ? "onb-brand-section-hash" : `onb-brand-section-${letter}`;
    ...
    <div className="space-y-4">
      {hidePageHeading ? null : (
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            <h2 className="profile-section-title">Favorite brands</h2>
            <p className="mt-1 text-sm text-white/60">
              This is the same brand picker — now part of setup
            </p>
          </div>
        </div>
      )}
      ...
      <div className="space-y-6">
        <label className="block w-full max-w-2xl" htmlFor="onb-brand-search">
        ...
        >
          <button
            ...
            className={`inline-flex min-h-[36px] min-w-[36px] items-center justify-center rounded-lg px-2 text-sm font-semibold transition ${
              savedOnly ? "bg-white text-black" : "text-white hover:bg-white/10"
            }`}
            title="Saved only"
            aria-pressed={savedOnly}
          >
            <HeartFilled className="h-5 w-5" title="Saved brands" />
          </button>
        ...
        {loading ? (
          ...
          <div className="space-y-10">
            {sections.map((letter) => {
              ...
              const sid =
                letter === "#"
                  ? "onb-brand-section-hash"
                  : `onb-brand-section-${letter}`;
              return (
    ...
    >
      ...
      >
        {onPortfolio ? (
          ...
          <HeartOutline className="h-5 w-5 text-white/50 group-hover:text-white/70" />
        )}
      </span>
